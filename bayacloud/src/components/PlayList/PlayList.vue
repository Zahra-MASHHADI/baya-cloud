  ...
  <div class="grid grid-cols-3 items-center gap-4 justify-bet">
    ...
    <div class="flex items-center gap-8 text-sm">
      <div
        ...
        v-if="+item.link > 0"
      >
        <span>{{ item.link }} لینک</span>
      </div>
      <div
        class="flex items-center gap-2 bg-gray-100 p-4 rounded-xl"
        v-if="+item.photo > 0"
      >
        <span>{{ item.photo }} عکس</span>
      </div>
    </div>
      ...
      <div class="flex gap-2 items-center justify-end">
        ...
        >
          <svg
            class="size-8"
            fill="none"
            :class="[item.like ? 'stroke-yellow-500 fill-yellow-500' : 'stroke-black']"
            viewBox="0 0 24 24"
          >
            <path
              stroke-linecap="round"
